...
def obtener_feeds_optimizados():
    # RESTAURANDO LAS ANTENAS MASIVAS DE GOOGLE NEWS
    return [
        ("https://gcaptain.com/feed/", "gCaptain (Naval)", "occidental"),
        ("https://feeds.bbci.co.uk/mundo/rss.xml", "BBC Mundo (UK)", "occidental"),
        ("https://news.google.com/rss/search?q=site:elpais.com+oriente+medio+OR+israel+OR+iran&hl=es-419&gl=CO&ceid=CO:es-419", "El País", "occidental"),
        ("https://news.google.com/rss/search?q=source:reuters+middle+east&hl=en-US&gl=US&ceid=US:en", "Reuters", "occidental"),
        ("https://news.google.com/rss/search?q=site:hispantv.com+israel+OR+iran+OR+eeuu+OR+guerra&hl=es-419&gl=CO&ceid=CO:es-419", "HispanTV (Irán)", "resistencia"),
        ("https://news.google.com/rss/search?q=site:en.irna.ir+israel+OR+iran+OR+military&hl=en-US&gl=US&ceid=US:en", "IRNA (Irán)", "resistencia"),
        ("https://www.aljazeera.com/xml/rss/all.xml", "Al Jazeera", "independiente"),
        ("https://actualidad.rt.com/rss", "RT (Rusia)", "alternativo"),
        ("https://news.google.com/rss/search?q=site:spanish.news.cn+israel+OR+iran+OR+oriente&hl=es-419&gl=CO&ceid=CO:es-419", "Xinhua (China)", "chino"),
        ("https://news.google.com/rss/search?q=site:timesofisrael.com+israel&hl=en-US&gl=US&ceid=US:en", "Times of Israel", "occidental"),
        ("https://news.google.com/rss/search?q=site:jpost.com+israel&hl=en-US&gl=US&ceid=US:en", "Jerusalem Post", "occidental")
    ]
...
def generar_mapa():
    print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Radar Geopolítico - MODO VOLUMEN EXTREMO")
    print("[*] Antenas de Google News restauradas. Límites de extracción eliminados.")
    print("[*] Advertencia: La compilación tomará varios minutos debido al alto volumen de inteligencia...")
    ...
    palabras_clave = [
        ...
        'eeuu', 'us', 'usa', 'estados unidos'
    ]
    
    mapa = folium.Map(location=[30.0, 40.0], zoom_start=5, tiles='CartoDB dark_matter')
    ...
    for url, agencia, bloque in feeds:
        print(f"[*] Escaneando antena masiva: {agencia}...")
        time.sleep(0.3)
        ...
        try:
            ...
            print(f"   [+] {len(flujo.entries)} transmisiones totales detectadas. Traduciendo todo el lote...")
            
            # --- SIN LÍMITES: LEE TODA LA BASE DE DATOS DE LA ANTENA ---
    ...
    <div style="position:fixed;top:20px;right:20px;width:240px;background:rgba(10,10,10,0.95);border:2px solid #444;padding:15px;border-radius:10px;font-family:'Courier New',monospace;font-size:11px;color:#fff;z-index:9999;box-shadow:0 0 15px #000;">
        <h4 style="color:#00ff41;margin:0 0 10px 0;text-align:center;font-size:13px;letter-spacing:1px;">🛰️ RADAR TÁCTICO EXTREMO</h4>
    ...
    print(f"\n{'='*60}")
    print(f"✅ BARRIDO TÁCTICO COMPLETADO CON VOLUMEN EXTREMO")
