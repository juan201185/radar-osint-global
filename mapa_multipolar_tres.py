...
def generar_mapa_multipolar():
    print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Iniciando Radar con DOBLE ENLACE Antibloqueo (Evasión Activa)...")
            ...
            for entrada in flujo.entries:
                ...
                if any(p in texto for p in palabras_clave):
                    ...
                    if coordenada_encontrada:
                        ...
                        if any(x in agencia for x in ['Irán', 'Rusia', 'China', 'IRNA', 'Xinhua', 'HispanTV']): color_pin = 'red'
                        elif any(x in agencia for x in ['Israel', 'Jerusalem', 'Reuters']): color_pin = 'blue'
                        elif 'gCaptain' in agencia: color_pin = 'lightgray'
                        else: color_pin = 'orange'

                        # --- CONSTRUCCIÓN DE BOTONES ---
                        # 1. Siempre creamos el botón verde (Directo)
                        botones_html = f"<a href='{url_original}' target='_blank' style='display: block; background-color: #00ff41; color: #000; padding: 10px; text-decoration: none; font-weight: bold; border-radius: 5px; margin-top: 12px; font-size: 12px;'>🔗 ENLACE DIRECTO</a>"
                        
                        # 2. Lógica Antibloqueo Inteligente
                        agencias_bloqueadas = ['HispanTV', 'IRNA', 'RT', 'Xinhua']
                        if any(censura in agencia for censura in agencias_bloqueadas):
                            if "news.google" in url_original:
                                # Si Google secuestró el enlace, evadimos buscando el titular en DuckDuckGo
                                titular_limpio = urllib.parse.quote_plus(titulo_es)
                                url_espejo = f"https://duckduckgo.com/?q={titular_limpio}"
                                texto_espejo = "🔍 RASTREAR TITULAR (EVADIR BLOQUEO)"
                            else:
                                # Si es un enlace puro (como RT), usamos el túnel forense
                                url_espejo = f"https://archive.ph/newest/{url_original}"
                                texto_espejo = "🛡️ TÚNEL ESPEJO (PROXY)"

                            botones_html += f"<a href='{url_espejo}' target='_blank' style='display: block; background-color: #ff4444; color: #fff; padding: 10px; text-decoration: none; font-weight: bold; border-radius: 5px; margin-top: 8px; font-size: 12px;'>{texto_espejo}</a>"

                        info_html = f"""
                        <div style="font-family: Arial; width: 240px; text-align: center;">
                            <b style="color: {color_pin}; font-size: 14px;">{agencia}</b><br>
                            <hr style="margin: 8px 0;">
                            <span style="font-size: 13px;">{titulo_es}</span><br>
                            {botones_html}
                        </div>
                        """
                        
                        folium.Marker(
                            location=coordenada_encontrada, 
                            popup=folium.Popup(info_html, max_width=260),
                            icon=folium.Icon(color=color_pin, icon='ship' if 'gCaptain' in agencia else 'info-sign')
                        ).add_to(agrupador_noticias)
                        impactos += 1
    ...
    leyenda_cibernetica = """
    <div style="position: fixed; bottom: 190px; left: 20px; width: 240px; z-index:9999; background-color: rgba(10,10,10,0.9); color: #00ff41; border: 1px solid #ff4444; padding: 15px; border-radius: 12px; font-family: 'Courier New', monospace; font-size: 11px; box-shadow: 0 0 15px #ff4444;">
        <h4 style="color:#ff4444; margin-top:0; margin-bottom: 8px; text-align: center; font-size: 13px;">⚠️ ALERTA DDoS ACTIVA</h4>
        <p style="text-align: justify; margin: 0; line-height: 1.4; color: #ccc;">Servidores bajo ataque. Si el enlace directo falla por error 504, use el botón <b>Rastrear Titular</b> o <b>Túnel Espejo</b> para evadir la censura.</p>
    </div>
    """
    mapa_osint.get_root().html.add_child(folium.Element(leyenda_cibernetica))
